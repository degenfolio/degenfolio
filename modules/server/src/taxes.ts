...
const log = getLogger(env.logLevel).child({ module: "Taxes",
  // level: "debug",
});
...
taxesRouter.post("/", async (req, res) => {
  ...
  if (trades.length) {
    ...
    for (let page = 0; page < nPages; page++) {
      ...
      const parseTrade = getCell => (trade: Trade, index: number): void => {
        ...
        log.info(
          `${trade.date.split("T")[0]} Sold ${pad(round(trade.quantity, 3))} ` +
  ...
  res.download("taxes/f8949.pdf", "f8949.pdf"); // empty form
});
